...
def run(args):
    from koopman_lm.evaluation.evaluate import load_model, eval_niah
    ...
    if "mqar" in tasks:
        from koopman_lm.evaluation.mqar.mqar import eval_mqar_grid
    ...
    if "ruler" in tasks:
        from koopman_lm.evaluation.ruler import eval_ruler_subset
        metrics["ruler"] = eval_ruler_subset(
            model, tokenizer, device,
            context_lens=tuple(plan["ruler_context_lens"]),
            n_examples=args.n_examples)
    if "babilong" in tasks:
        from koopman_lm.evaluation.babilong import eval_babilong_subset
