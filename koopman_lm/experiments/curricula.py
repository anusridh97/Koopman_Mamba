...
Shared by both experiments in this package:

    - mqar_finetune.py (Section 5.2): 50M in-task MQAR fine-tuning, one cell
      trained+evaluated independently per (model_type, KV pairs, distractor
      gap).
    - table2.py (Table 2, Section 4.1): "needle-in-a-haystack retrieval
      (KV=1)" is this same generator special-cased to num_kv_pairs=1 -- the
      paper gives no separate NIAH token format, and frames Table 2's task
      explicitly as "(KV=1)" while noting "each task varies KV pairs and
      sequence length," so KV=1 is read here as the MQAR family's smallest
      cell rather than a bespoke needle format.

Token-id-level synthetic: keys and values live in disjoint halves of the
vocab. A sequence lists key-value pairs, then re-presents a subset of keys
as queries; the model must predict each queried key's value at the
following position. Sparse supervision (labels are -100 except at the
answer positions) matches the paper's own MQAR protocol (Sec 5.2: "trained
