...
class KoopmanLMConfig:
    ...
    # Layer layout (stored as a tuple so the frozen config stays hashable/immutable)
    ...
    def __post_init__(self):
        # frozen dataclass: must set fields via object.__setattr__ in post_init
        if self.ska_layer_indices is None:
            object.__setattr__(self, "ska_layer_indices", (4, 8, 12, 16, 20, 23))
        else:
            # accept any sequence (factories pass lists) -> normalize to tuple
            object.__setattr__(self, "ska_layer_indices", tuple(self.ska_layer_indices))
        # normalize gamma clamp to a tuple (or None) for stable hashing
...
# ============================================================================
# Config hashing — content-addressed, stable across processes
# ============================================================================

def config_hash(cfg: "KoopmanLMConfig") -> str:
    """Stable SHA-256 hex digest of a config's contents.

    Computed from canonical JSON (sorted keys) of ``dataclasses.asdict``, so it
    is deterministic across processes (unlike Python's salted ``hash()``).
    Stored in every checkpoint and used to tag wandb run groups, e.g.
    ``f"{tag}-{model_size}-{config_hash(cfg)[:8]}"``.
    """
...
def config_180m_gated():
    return replace(config_180m(), mlp_gated=True)
...
# ============================================================================
# Production scaling policy (440M+): eta=gamma=1 fixed, LayerScale residual,
# short-range causal conv ON. Shared by config_440m / 880m / 1p5b / 3b.
# ============================================================================

def _production_policy():
    """Kwargs for the 440M+ scale-parameter policy (see KoopmanLMConfig docs)."""
    return dict(
...
def config_440m():
    """Koopman LM 440M: 26 Mamba-2 + 8 SKA, 34 Koopman MLP (~444M params).
    ...
    eta=gamma=1 fixed; LayerScale residual.
    """
    return KoopmanLMConfig(
        d_model=1024,
        n_layers=34,
...
def config_880m():
    """Koopman LM ~880M: 28 Mamba-2 + 9 SKA, 37 layers (~882M params).

    d=1408 (head_dim 64), rank 96. ~24% SKA, evenly spaced. Production policy.
    """
    return KoopmanLMConfig(
        d_model=1408,
        n_layers=37,
        ...
        ska_rank=96,
        ska_chunk_size=128,
        ska_layer_indices=_evenly_spaced_indices(37, 9),
        mlp_gated=False,
        **_production_policy(),
    )
...
def config_1p5b():
    """Koopman LM ~1.5B: 30 Mamba-2 + 10 SKA, 40 layers (~1.55B params).

    d=1792 (head_dim 64), rank 128. 25% SKA, evenly spaced. Production policy.
    """
    return KoopmanLMConfig(
        d_model=1792,
        n_layers=40,
        ...
        ska_rank=128,
        ska_chunk_size=128,
        ska_layer_indices=_evenly_spaced_indices(40, 10),
        mlp_gated=False,
        **_production_policy(),
    )
...
def config_3b():
    """Koopman LM ~3B: 36 Mamba-2 + 12 SKA, 48 layers (~3.0B params).

    d=2304 (head_dim 64), rank 128. 25% SKA, evenly spaced. Production policy.
    """
    return KoopmanLMConfig(
        d_model=2304,
        n_layers=48,
        ...
        ska_rank=128,
        ska_chunk_size=128,
        ska_layer_indices=_evenly_spaced_indices(48, 12),
        mlp_gated=False,
        **_production_policy(),
    )
...
# Registry: model_size string -> factory. Single source of truth for train /
# eval CLIs so a new scale only needs to be added here.
CONFIG_FACTORIES = {
    "50m": config_50m,
    "180m": config_180m,
    "180m_gated": config_180m_gated,
    "370m": config_370m,
    "440m": config_440m,
    "880m": config_880m,
    "1p5b": config_1p5b,
...
def build_config(model_size: str) -> "KoopmanLMConfig":
    """Look up a config factory by model_size string (raises on unknown)."""
    if model_size not in CONFIG_FACTORIES:
        raise ValueError(
            f"Unknown model_size: {model_size!r}. "
            f"Known: {sorted(CONFIG_FACTORIES)}"
        )
    return CONFIG_FACTORIES[model_size]()
