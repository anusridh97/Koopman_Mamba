...
`expand_cells`), and koopman_lm/run/slurm.py's array job only ever reads the
...
from koopman_lm.run.resolve import load_raw_spec, resolve_model_config
from koopman_lm.run.spec import OptimSpec, RunSpec, RuntimeSpec, data_spec_from_dict
    ...
    def __post_init__(self):
        ...
        if self.axes and self.cells is not None:
            raise ValueError(
                ...
                "one place must declare the grid (see koopman_lm.sweep.spec "
...
def load_sweep_spec(path) -> SweepSpec:
    ...
    process's cwd -- the same convention `python -m koopman_lm.run
...
def sweep_id(sweep: SweepSpec) -> str:
    ...
    (sweep_id + sweep_name) so `python -m koopman_lm.results` can group by
...
def build_cell_run_spec(name: str, sections: Dict[str, Dict[str, Any]],
    ...
    -- it plays no part in run_id/group_id (koopman_lm.run.spec hashes only
...
def expand_cells(sweep: SweepSpec) -> List[SweepCell]:
    ...
    koopman_lm.run.spec machinery a lone `python -m koopman_lm.run` launch
