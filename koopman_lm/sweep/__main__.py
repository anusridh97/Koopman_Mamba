"""python -m koopman_lm.sweep <sweep.yaml> [--launcher local|slurm]
[--dry_run] [--skip_done] [--run_root PATH] [--force] [--allow-dirty]

Expands configs/sweeps/<name>.yaml (§4.2) into one RunSpec per cell via
koopman_lm.sweep.spec.expand_cells -- the single place the grid is declared
-- then drives each surviving cell through the same primitives
`python -m koopman_lm.run` uses per-run: koopman_lm.run.resolve/artifacts/
launch. This is a NEW entry point, deliberately not a modification of
koopman_lm/run/__main__.py (a separate agent owns that file's SIGUSR1 work).

Orchestration order, per invocation:
    expand the sweep into cells (koopman_lm.sweep.spec.expand_cells)
    ...
    for each surviving cell: verify data, create_run_dir, materialize
      ...
      attempt record -- byte-for-byte what koopman_lm.run.__main__ does for
      a single run
    hand off every surviving cell to a Launcher:
      ...
      --launcher slurm  one SlurmLauncher.submit_array() for the whole
                         sweep (koopman_lm/run/slurm.py's array-job support,
...
from koopman_lm.run.resolve import check_git_clean, git_commit, materialize
...
from koopman_lm.sweep.spec import SweepCell, SweepSpec, expand_cells, load_sweep_spec
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(prog="python -m koopman_lm.sweep")
...
def _print_plan(sweep: SweepSpec, cells: List[SweepCell], run_root) -> None:
    ...
    print(f"[koopman_lm.sweep] {sweep.name} sweep_id={sid}  {len(cells)} cell(s)")
    for cell in cells:
...
def main(argv=None):
    ...
    if args.skip_done and len(pending) < len(cells):
        print(f"[koopman_lm.sweep] --skip_done: "
              f"{len(cells) - len(pending)} already-done cell(s) skipped, "
              f"{len(pending)} remaining")
    if not pending:
        print("[koopman_lm.sweep] nothing to do (every cell is done)")
        return []
