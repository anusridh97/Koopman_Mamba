...
def _ppl(model, device, tokenizer, plan):
    from koopman_lm.evaluate import eval_held_out_ppl
...
def run(args):
    from koopman_lm.evaluate import load_model, eval_niah
    ...
    if "mqar" in tasks:
        from koopman_lm.evals.mqar import eval_mqar_grid
    ...
    if "ruler" in tasks:
        from koopman_lm.evals.ruler import eval_ruler_subset
