...
The current Python/Torch implementation is intentionally a reference backend:
its forward has the exact blocked scan structure and its custom backward uses
an analytic reverse prefix scan.  The CUDA implementation should fuse the same
phases; no mathematical changes are required.
...
def ska_prefix_scan(
    ...
    This reference executes the intended blocked forward and an analytic scan
    backward.  Production CUDA should preserve this API and replace the two
    internal phases with fused kernels.
    """
