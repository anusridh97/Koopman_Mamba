...
| `C₊`/`R₊` induction oracle | **Landed** (`code-tests/test_induction_cplus_oracle.py`); **product stage post-Gate-2** | I1 shifted-ridge optimality vs an independent augmented-QR solve + shared-`G̃` 1/T slack (monotone in the mean, <5e-3 at T≫1); I2 v-endpoint boundary combine (non-const β teeth vs 4 wrong weightings); I3 `R₊` one-sided drag-along reusing `w_t=L⁻¹x_{t-1}`, matches fresh `C₊L⁻ᵀ` at 1e-9. Adds `Pr` state + touches write path → its own post-Gate-2 product stage |
...
| Causal norm-clip (memo §6) | **Landed behind flag** (`ska_norm_clip`, default off = ℓ2); 5-site via `causal_normalize` helper; `test_norm_clip_torch.py` | Properties verified (leverage-bounded, low-norm preserved, ℓ2 inflates, contractive under clip). Behavior change → **its own before/after eval still required**, and **must precede the Gate-2 gate arms** (the gate reads the post-normalization query) |
