...
# load_model moved to loader.py (it was duplicated here and in
# evaluate_retrieval.py); re-imported at module level because
# experimentation/evaluation/harness.py:164 imports it from here.
...
def find_run_dir(checkpoint_path):
    ...
    spec.yaml (experimentation.run.resolve.materialize's marker for a run
    directory). Returns the run_dir Path, or None if the checkpoint isn't
    inside one (e.g. a hand-run checkpoint outside experimentation.run) -- callers
...
def write_checkpoint_result(checkpoint_path, task, metrics):
    ...
    (experimentation.results' `eval_git_commit` column distinguishes the two).
...
def eval_fineweb_ppl(model, device, held_out_data_dir, max_seq_len=2048,
    ...
    experimentation.training.data.pretokenize's --skip_docs for how to produce a
...
def main():
    ...
    if args.checkpoint2:
        ...
        # Inside a experimentation.run run directory: always wire into that run's
