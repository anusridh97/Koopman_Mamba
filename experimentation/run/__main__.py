"""python -m experimentation.run <spec.yaml> [--launcher local|slurm] [--resume]
...
from experimentation.run.resolve import check_git_clean, git_commit, materialize, resolve_run_spec
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(prog="python -m experimentation.run")
...
def main(argv=None):
    ...
    print(f"[experimentation.run] run_id={compute_run_id(spec)} run_dir={run_dir}")
    return result
