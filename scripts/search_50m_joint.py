...
"""The 50M joint architecture + optimizer search, as one standalone driver.
    ...
    python scripts/search_50m_joint.py configs/search/50m-joint-v1.yaml --dry_run
    python scripts/search_50m_joint.py configs/search/50m-joint-v1.yaml
    python scripts/search_50m_joint.py <study> --report_only   # re-read a journal
...
Fixed by the base RunSpec (`configs/runs/50m-joint-base.yaml`) and by this study
...
searched there is no "the base architecture": `configs/runs/50m-joint-base.yaml`
...
PARAM_BAND = (45_500_000, 54_500_000)
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        description="50M joint architecture + optimizer search",
        formatter_class=argparse.RawDescriptionHelpFormatter)
    p.add_argument("study", help="path to configs/search/50m-joint-*.yaml")
...
def _fanout(args, spec, *, gpus, log_dir):
    ...
    for index in range(spec.concurrent_trials):
        ...
        procs.append(proc)
        print(f"[50m-joint]   worker {index}  "
    ...
    for index, code in enumerate(codes):
        if code != 0:
            print(f"[50m-joint] worker {index} exited {code}")
...
def _print_plan(spec, space, base_model, base_sections, *, study_dir, gpus,
    ...
    lo, hi = envelope
    print(f"[50m-joint] study      {spec.name}")
    print(f"[50m-joint] base       {spec.base}")
    print(f"[50m-joint] study_dir  {study_dir}")
    print(f"[50m-joint] trials     {spec.n_trials} x {spec.max_steps} steps, "
          f"{spec.concurrent_trials} worker(s), GPUs {gpus or '(none)'}")
    print(f"[50m-joint] tokens     {tokens:,} per trial "
          f"({tokens / max(1, base_model.param_count_estimate()):.1f} per base param)")
    print(f"[50m-joint] param band {PARAM_BAND[0]:,}-{PARAM_BAND[1]:,}; "
          f"space spans {lo:,}-{hi:,} "
          f"({100.0 * (hi - lo) / lo:.1f}% spread)")
    print(f"[50m-joint] backend    {spec.backend_policy}   "
          f"sampler {spec.sampler} (startup {spec.sampler_startup_trials})")
    print(f"[50m-joint] pruning    after step {spec.prune_after_step} of "
          f"{spec.max_steps}, once {spec.prune_startup_trials} trials complete")
    print("[50m-joint] macro axes (declared in this script, hashed into study_dir):")
    for name, decl in MACRO_AXES.items():
        print(f"[50m-joint]   {name:16} {decl['choices']}")
    print("[50m-joint] derived depth, and the mean parameter count of each cell:")
    for (expand, tier), (depth, mean) in sorted(tier_table(base_model, space).items()):
        print(f"[50m-joint]   expand {expand}  {tier:5}  depth {depth:2}  "
              f"{mean:>11,.0f} mean parameters")
    print("[50m-joint] declared axes (space.py, restricted by the study file):")
    for name in sorted(space):
        ...
        print(f"[50m-joint]   {name:22} {shown}")
    dims = len(MACRO_AXES) + len(space)
    print(f"[50m-joint] dimensions {dims} "
...
def _write_outputs(study, study_dir, spec, base_model):
    ...
    for name, path in sorted(written.items()):
        if path is not None:
            print(f"[50m-joint] wrote {name:16} {path}")
    ...
    print(f"[50m-joint] wrote {'macro_pairwise':16} {path}")
...
def main(argv=None):
    ...
    if not is_worker:
        ...
        print(f"[50m-joint] band check {checked} size-bearing combination(s), "
              f"{len(bad)} violation(s)")
        if bad:
            for row in bad[:10]:
                print(f"[50m-joint]   REJECTED {row}")
            raise SystemExit(
    ...
    if spec.design_file:
        ...
        if not is_worker:
            ...
            print(f"[50m-joint] anchors    {len(designs)} from {spec.design_file}, "
                  f"{len(resolved)} resolve")
            if groups:
                for name, members in sorted(groups.items()):
                    seeds = [getattr(m, "seed", "baseline") for m in members]
                    print(f"[50m-joint]   noise floor '{name}': {len(members)} "
                          f"replicates, seeds {seeds}")
            else:
                print("[50m-joint]   WARNING: no reference_group -- "
                      "analysis.noise_floor will report nothing, exactly as the "
                      "3M study did. Every effect would be UNCALIBRATED.")
    elif not is_worker:
        print("[50m-joint] anchors    none -- no design_file, so there will be "
              "NO noise floor (see this script's docstring)")

    if args.dry_run:
        print("[50m-joint] --dry_run: nothing materialized, nothing submitted.")
        return 0
    ...
    if designs and not is_worker:
        added = _enqueue_macro_anchors(
            ...
            base_lr=float(base_sections["optim"]["lr"]))
        print(f"[50m-joint] enqueued {added} anchor(s) "
              f"({len(designs) - added} already present)")

    if should_fanout(spec.concurrent_trials, raw_worker):
        print(f"[50m-joint] launching {spec.concurrent_trials} worker(s)")
    ...
    while _terminal_count(study) < spec.n_trials:
        ...
        if stalled >= 3:
            raise SystemExit(
                f"[50m-joint] {stalled} trials finished without advancing the "
    ...
    print(f"[50m-joint] {sum(states.values())} trial(s): "
