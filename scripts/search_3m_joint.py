#!/usr/bin/env python3
"""The 3M joint architecture + optimizer search, as one standalone driver.

    python scripts/search_3m_joint.py configs/search/3m-joint-v1.yaml --dry_run
    python scripts/search_3m_joint.py configs/search/3m-joint-v1.yaml
    python scripts/search_3m_joint.py <study> --report_only   # re-read a journal
...
this file exists for -- "at a fixed ~3M parameter budget and a fixed token
...
At 3M total parameters with a tied 32K-vocab embedding, WIDTH IS NOT FREE. The
embedding alone is `vocab * d_model`, so d_model=64 spends 2.048M of the budget
and leaves ~0.95M for everything else. Enumerated against
`KoopmanLMConfig.param_count_estimate()` over the whole joint space, the only
widths with a viable depth are:

    d_model  expand   depths entirely inside 2.8M-3.25M
       64      1      15, 16, 17
       64      2      12, 13, 14
       64      3      10, 11
       80      1      3, 4, 5          <- too shallow: see below
       80      2      3, 4
       80      3      2, 3, 4
       32      *      none (would need ~94 layers)

d_model=80 is excluded even though it is in band. `geometry.make_layer_indices`
reserves layer 0 and the final layer, so a 4-layer backbone has a 2-slot window:
every SKA layer count clamps to 2 and all placements resolve to the SAME indices.
Three of the axes below would be dead, and a parameter importance would be
computed over a difference that does not exist. That is the exact failure
`configs/search/proxy-256x17-interactions-v1.yaml` was moved off 4m-golden to
avoid, and it is why width is FIXED at 64 rather than searched.

What is left is the interesting part: at 3M, depth and Mamba expansion trade off
against each other at constant parameter count. So the axes are `mamba_expand`
and a two-level `depth_tier`, and DEPTH IS DERIVED from the pair (see `DEPTHS`).
Sampling depth independently would propose (expand 3, depth 17) = 3.9M and spend
a quarter of the study rejecting its own draws.
...
Fixed by the base RunSpec (`configs/runs/3m-joint-base.yaml`) and by this study
...
It does not enqueue anchors and it has no design file. A 2000-trial random-start
study does not need 28 one-factor-at-a-time screens to have a reference
population, and the noise floor it would otherwise buy is better bought as
explicit seed replicates of the promoted configs afterwards -- which is a
separate, cheap, post-search job. `analysis.noise_floor` will therefore report
nothing for this study, and every effect it prints is UNCALIBRATED until those
replicates exist. Read `shortlist`/`main_effects` accordingly.
...
searched there is no "the base architecture": `configs/runs/3m-joint-base.yaml`
...
#:     lean   2.906M (15)   2.903M (12)   2.901M (10)   0.2%
#:     full   2.959M (16)   2.969M (13)   2.980M (11)   0.7%
#:
...
#:
#: Depths 14 and 17 are in band and are deliberately EXCLUDED: their cells have
#: no expansion-3 partner (expansion 3 runs out of band at depth 12), so
#: including them would make the expansion axis confounded with size again.
DEPTHS = {
    (1, "lean"): 15, (1, "full"): 16,
    (2, "lean"): 12, (2, "full"): 13,
    (3, "lean"): 10, (3, "full"): 11,
...
MACRO_AXES = {
    ...
    # SKA head geometry. head_dim = 64 // ska_n_heads, so this is {64, 32, 16, 8}
...
PARAM_BAND = (2_800_000, 3_250_000)
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        description="3M joint architecture + optimizer search",
        formatter_class=argparse.RawDescriptionHelpFormatter)
    p.add_argument("study", help="path to configs/search/3m-joint-*.yaml")
...
def _fanout(args, spec, *, gpus, log_dir):
    ...
    for index, code in enumerate(codes):
        if code != 0:
            print(f"[3m-joint] worker {index} exited {code}")
...
def _print_plan(spec, space, base_model, base_sections, *, study_dir, gpus,
    ...
    lo, hi = envelope
    print(f"[3m-joint] study      {spec.name}")
    print(f"[3m-joint] base       {spec.base}")
    print(f"[3m-joint] study_dir  {study_dir}")
    print(f"[3m-joint] trials     {spec.n_trials} x {spec.max_steps} steps, "
          f"{spec.concurrent_trials} worker(s), GPUs {gpus or '(none)'}")
    print(f"[3m-joint] tokens     {tokens:,} per trial "
          f"({tokens / max(1, base_model.param_count_estimate()):.1f} per base param)")
    print(f"[3m-joint] param band {PARAM_BAND[0]:,}-{PARAM_BAND[1]:,}; "
          f"space spans {lo:,}-{hi:,} "
          f"({100.0 * (hi - lo) / lo:.1f}% spread)")
    print(f"[3m-joint] backend    {spec.backend_policy}   "
          f"sampler {spec.sampler} (startup {spec.sampler_startup_trials})")
    print(f"[3m-joint] pruning    after step {spec.prune_after_step} of "
          f"{spec.max_steps}, once {spec.prune_startup_trials} trials complete")
    print("[3m-joint] macro axes (declared in this script, hashed into study_dir):")
    for name, decl in MACRO_AXES.items():
        print(f"[3m-joint]   {name:16} {decl['choices']}")
    print("[3m-joint] derived depth, and the mean parameter count of each cell:")
    for (expand, tier), (depth, mean) in sorted(tier_table(base_model, space).items()):
        print(f"[3m-joint]   expand {expand}  {tier:5}  depth {depth:2}  "
              f"{mean:>11,.0f} mean parameters")
    print("[3m-joint] declared axes (space.py, restricted by the study file):")
    for name in sorted(space):
        ...
        print(f"[3m-joint]   {name:22} {shown}")
    dims = len(MACRO_AXES) + len(space)
    print(f"[3m-joint] dimensions {dims} "
...
def _write_outputs(study, study_dir, spec, base_model):
    ...
    for name, path in sorted(written.items()):
        if path is not None:
            print(f"[3m-joint] wrote {name:16} {path}")
    ...
    print(f"[3m-joint] wrote {'macro_pairwise':16} {path}")
...
def main(argv=None):
    ...
    if not is_worker:
        ...
        print(f"[3m-joint] band check {checked} size-bearing combination(s), "
              f"{len(bad)} violation(s)")
        if bad:
            for row in bad[:10]:
                print(f"[3m-joint]   REJECTED {row}")
            raise SystemExit(
    ...
    if args.dry_run:
        print("[3m-joint] --dry_run: nothing materialized, nothing submitted.")
        return 0
    ...
    if should_fanout(spec.concurrent_trials, raw_worker):
        print(f"[3m-joint] launching {spec.concurrent_trials} worker(s)")
    ...
    while _terminal_count(study) < spec.n_trials:
        ...
        if stalled >= 3:
            raise SystemExit(
                f"[3m-joint] {stalled} trials finished without advancing the "
    ...
    print(f"[3m-joint] {sum(states.values())} trial(s): "
