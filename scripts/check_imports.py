...
test covered:
  ...
  * scripts/inspect_checkpoint.py imported build_mamba_attention / build_mamba_only
    from koopman_lm.evaluation.evaluate. The script came from phase1-finalize, whose
...
def _exported_names(body: list[ast.stmt]) -> set[str]:
    ...
    `from koopman_lm.evaluation.evaluate import build_mamba_attention` resolvable.)
...
def main() -> int:
    ...
    for path in glob.glob(f"{PACKAGE}/**/*.py", recursive=True):
        tree = _parse(path)
        if tree is not None:
            modules[_module_name(path)] = _exported_names(tree.body)

    if not modules:
        print(f"ERROR: no {PACKAGE}/ modules found -- run this from the repo root.",
              file=sys.stderr)
        return 1
    ...
    for path in sources:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                ...
                if node.level or not node.module or not node.module.startswith(PACKAGE):
                    continue
    ...
    if not args.quiet:
        print(f"indexed {len(modules)} {PACKAGE} modules; scanned {len(sources)} files")
