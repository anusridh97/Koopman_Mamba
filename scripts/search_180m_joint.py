#!/usr/bin/env python3
"""The 50M joint architecture + optimizer search, as one standalone driver.

    python scripts/search_180m_joint.py configs/search/180m-joint-v1.yaml --dry_run
    python scripts/search_180m_joint.py configs/search/180m-joint-v1.yaml
    python scripts/search_180m_joint.py <study> --report_only   # re-read a journal
...
this file exists for -- "at a fixed ~50M parameter budget and a fixed token
...
At 50M TOTAL parameters the width choice is a trade-off rather than a corner to
avoid. Four widths reach the band with a tied 32K embedding:

    d_model  embedding  in-band depths (e1/e2/e3)  one layer is
      256       16%       49-59 / 39-47 / 32-39       1.5% of 50M
      320       20%       29-36 / 23-28 / 19-23       2.3%
      384       25%       20-24 / 16-19 / 13-15       3.3%   <- CHOSEN
      448       29%       13-16 / 11-13 /  9-10       4.5%

None of these hits the dead-axis failure that ruled out d_model 192 at 10M and
80 at 3M -- even d_model 448 keeps depth >= 9, whose usable SKA window is 7 slots,
so `n_ska_layers: 6` still resolves distinctly. The binding consideration here is
different: narrower widths give a finer depth grid and so a finer `depth_tier`
step, but a WORSE within-tier match. Within-tier spread is what confounds
`mamba_expand`, which carried 40.2% of the variance in the 3M good-LR band, so
it is the quantity to minimise:

    d_model 384, lean (22,17,14) full (23,18,15): spread 1.10%/0.94%, step +4.23%
    d_model 320, lean (32,25,21) full (33,26,22): spread 1.22%/1.81%, step +3.05%

384 buys a 0.94% iso-parameter contrast on the axis that matters, uses 40% fewer
sequential layers (these models are latency-bound, not compute-bound -- the 10M
ran at 0.6% of an H100's bf16 peak), and matches `configs/50m.yaml` so the
measured 111,254 tok/s transfers directly.

Depth is still DERIVED from (mamba_expand, depth_tier). Sampling it freely would
propose (expand 3, depth 23) = 78M and spend much of the study rejecting its own
draws.
...
Fixed by the base RunSpec (`configs/runs/180m-joint-base.yaml`) and by this study
...
searched there is no "the base architecture": `configs/runs/180m-joint-base.yaml`
...
#:     lean   50.139M (22)    49.591M (17)    49.626M (14)   1.10%
#:     full   51.792M (23)    51.698M (18)    52.187M (15)   0.94%
#:
#: The tier STEP is +4.23%, coarser than 3M's ~2% or 10M's ~2.6%, and that is a
#: granularity limit rather than a choice: one layer at d_model 384 is 3.3% of a
#: 50M model, so no depth table can put two matched tiers closer. `depth_tier` is
#: therefore a coarser probe of the loss-per-parameter slope at this rung. The
#: within-tier match is BETTER than at 10M (0.94% against 1.46%), which is the
#: trade taken deliberately -- see the width table in the module docstring.
#:
...
#:
#: The searched axes swing the parameter count by 3.28M -- 6.6% of the base,
#: smaller than 10M's 10.3% because the embedding is a smaller share here.
#: `--dry_run` enumerates the exhaustive size-bearing product (1,920
#: combinations) and confirms 0 violations over a 48.500M-54.358M span (12.1%).
DEPTHS = {
    # d_model 640. Chosen over 512/576/704/768 because it is the geometry
    # `configs/180m.yaml` already ships, so the measured 46,372 tok/s at
    # microbatch 12 transfers directly rather than needing a fresh probe -- the
    # same argument that picked 384 at 50M.
    #
    # Selected by minimising WITHIN-TIER parameter spread, which is what
    # confounds `mamba_expand`, then centring on the nominal 176.3M. The
    # alternative lean(34,27,22) matched marginally better (1.60% vs 1.67%) but
    # sat +4.6% above nominal against this table's +1.3%:
    #
    #     expand 1  lean 33 -> 175.1M   full 34 -> 180.9M
    #     expand 2  lean 26 -> 176.8M   full 27 -> 182.7M
    #     expand 3  lean 21 -> 173.9M   full 22 -> 181.0M
    #
    # within-tier spread 1.67% (lean) / 0.95% (full); tier step +3.23%. The step
    # is a GRANULARITY LIMIT, not a choice: one layer at d_model 640 is 3.2% of
    # a 180M model, so no depth table can put matched tiers closer. Read the
    # loss-per-parameter slope `depth_tier` measures accordingly -- it is
    # coarser here than at 3M (~2%) and 10M (~2.6%), and comparable to 50M.
    (1, "lean"): 33,
    (1, "full"): 34,
    (2, "lean"): 26,
    (2, "full"): 27,
...
PARAM_BAND = (171_000_000, 187_000_000)
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        description="180M joint architecture + optimizer search",
        formatter_class=argparse.RawDescriptionHelpFormatter)
    p.add_argument("study", help="path to configs/search/180m-joint-*.yaml")
...
def _fanout(args, spec, *, gpus, log_dir):
    ...
    for index in range(spec.concurrent_trials):
        ...
        procs.append(proc)
        print(f"[180m-joint]   worker {index}  "
    ...
    for index, code in enumerate(codes):
        if code != 0:
            print(f"[180m-joint] worker {index} exited {code}")
...
def _print_plan(spec, space, base_model, base_sections, *, study_dir, gpus,
    ...
    lo, hi = envelope
    print(f"[180m-joint] study      {spec.name}")
    print(f"[180m-joint] base       {spec.base}")
    print(f"[180m-joint] study_dir  {study_dir}")
    print(f"[180m-joint] trials     {spec.n_trials} x {spec.max_steps} steps, "
          f"{spec.concurrent_trials} worker(s), GPUs {gpus or '(none)'}")
    print(f"[180m-joint] tokens     {tokens:,} per trial "
          f"({tokens / max(1, base_model.param_count_estimate()):.1f} per base param)")
    print(f"[180m-joint] param band {PARAM_BAND[0]:,}-{PARAM_BAND[1]:,}; "
          f"space spans {lo:,}-{hi:,} "
          f"({100.0 * (hi - lo) / lo:.1f}% spread)")
    print(f"[180m-joint] backend    {spec.backend_policy}   "
          f"sampler {spec.sampler} (startup {spec.sampler_startup_trials})")
    print(f"[180m-joint] pruning    after step {spec.prune_after_step} of "
          f"{spec.max_steps}, once {spec.prune_startup_trials} trials complete")
    print("[180m-joint] macro axes (declared in this script, hashed into study_dir):")
    for name, decl in MACRO_AXES.items():
        print(f"[180m-joint]   {name:16} {decl['choices']}")
    print("[180m-joint] derived depth, and the mean parameter count of each cell:")
    for (expand, tier), (depth, mean) in sorted(tier_table(base_model, space).items()):
        print(f"[180m-joint]   expand {expand}  {tier:5}  depth {depth:2}  "
              f"{mean:>11,.0f} mean parameters")
    print("[180m-joint] declared axes (space.py, restricted by the study file):")
    for name in sorted(space):
        ...
        print(f"[180m-joint]   {name:22} {shown}")
    dims = len(MACRO_AXES) + len(space)
    print(f"[180m-joint] dimensions {dims} "
...
def _write_outputs(study, study_dir, spec, base_model):
    ...
    for name, path in sorted(written.items()):
        if path is not None:
            print(f"[180m-joint] wrote {name:16} {path}")
    ...
    print(f"[180m-joint] wrote {'macro_pairwise':16} {path}")
...
def main(argv=None):
    ...
    # A study whose name still carries the PROVISIONAL suffix has fields that were
    # left open on purpose and must be filled from measurement before it can run.
    # `--dry_run` and `--report_only` are allowed so the space, the parameter band
    # and the anchors can all be verified on a laptop while those fields are still
    # open; only the launch is blocked.
    #
    # The suffix lives in `name`, which `studyspec.study_id` hashes, so filling the
    # fields and dropping the suffix produces a DIFFERENT study_id and therefore a
    # different journal. That is deliberate: a provisional study can never be
    # resumed into the real one, and a half-specified run cannot contaminate it.
    if spec.name.endswith("-PROVISIONAL") and not (args.dry_run or args.report_only):
        raise SystemExit(
            f"{args.study}: study name is '{spec.name}'. The -PROVISIONAL suffix "
            f"means at least one field is still a placeholder awaiting a "
            f"measurement -- for this study, `ska_rank.choices` and "
            f"`learning_rate.low/high`, which must come from the 10M anchor "
            f"contrasts because BOTH won at the ceiling of their 3M range and a "
            f"boundary win is a lower bound rather than a location.\n"
            f"Fill them, drop the suffix from `name`, and re-run --dry_run to "
            f"confirm the band and the anchors still resolve.")
    ...
    if not is_worker:
        ...
        print(f"[180m-joint] band check {checked} size-bearing combination(s), "
              f"{len(bad)} violation(s)")
        if bad:
            for row in bad[:10]:
                print(f"[180m-joint]   REJECTED {row}")
            raise SystemExit(
    ...
    if spec.design_file:
        ...
        if not is_worker:
            ...
            print(f"[180m-joint] anchors    {len(designs)} from {spec.design_file}, "
                  f"{len(resolved)} resolve")
            if groups:
                for name, members in sorted(groups.items()):
                    seeds = [getattr(m, "seed", "baseline") for m in members]
                    print(f"[180m-joint]   noise floor '{name}': {len(members)} "
                          f"replicates, seeds {seeds}")
            else:
                print("[180m-joint]   WARNING: no reference_group -- "
                      "analysis.noise_floor will report nothing, exactly as the "
                      "3M study did. Every effect would be UNCALIBRATED.")
    elif not is_worker:
        print("[180m-joint] anchors    none -- no design_file, so there will be "
              "NO noise floor (see this script's docstring)")

    if args.dry_run:
        print("[180m-joint] --dry_run: nothing materialized, nothing submitted.")
        return 0
    ...
    if designs and not is_worker:
        added = _enqueue_macro_anchors(
            ...
            base_lr=float(base_sections["optim"]["lr"]))
        print(f"[180m-joint] enqueued {added} anchor(s) "
              f"({len(designs) - added} already present)")

    if should_fanout(spec.concurrent_trials, raw_worker):
        print(f"[180m-joint] launching {spec.concurrent_trials} worker(s)")
    ...
    while _terminal_count(study) < spec.n_trials:
        ...
        if stalled >= 3:
            raise SystemExit(
                f"[180m-joint] {stalled} trials finished without advancing the "
    ...
    print(f"[180m-joint] {sum(states.values())} trial(s): "
